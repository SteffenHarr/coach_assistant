 ...
 */
export function PlanCalendar({ sessions, coachNames, courtNames, playerNames }: Props) {
  // Group sessions by weekday for placement.
  const byDay = useMemo(() => {
    const out: Session[][] = Array.from({ length: 7 }, () => []);
    for (const s of sessions) {
      if (!s.slot_indices.length) continue;
  ...
  return (
    ...
    >
      ...
      {Array.from({ length: SLOTS_PER_DAY }).map((_, row) => (
        <RowFragment key={row} row={row} byDay={byDay} coachNames={coachNames} courtNames={courtNames} playerNames={playerNames} />
      ))}
    </div>
  );
}
...
function RowFragment({
  ...
  return (
    <>
      ...
      {WEEKDAYS.map((_d, day) => {
        // Find a session whose first slot is exactly at this (day,row)
        const idx = slotIndex(day, row);
        const starting = byDay[day].filter((s) => s.slot_indices[0] === idx);
        const continuing = byDay[day].some(
          (s) => s.slot_indices.includes(idx) && s.slot_indices[0] !== idx,
        );
        return (
          <div
            key={day}
